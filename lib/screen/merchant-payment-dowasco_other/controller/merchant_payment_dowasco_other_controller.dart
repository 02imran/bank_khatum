...
class MerchantPaymentDowascoOtherController extends GetxController with StateMixin {
  final GlobalKey<FormState> cashInFromKey = GlobalKey<FormState>();
  ...
  var reference = "Water Connection".obs;
  ...
  void onInit() {
    ...
    String merchantName=Get.arguments[0];
    String merchantCode=Get.arguments[1];
    String amount=Get.arguments[2];
    ...
    nameController.text=merchantName;
    phoneController.text=merchantCode;
  ...
  String? validatePhone(String value) {
    if (value.length < 4) {
      return "Phone number Or Merchant Code  not valid";
    }
    return null;
  }
  ...
  checkMerchantPayment(BuildContext context) {
    final isValid = cashInFromKey.currentState!.validate();
    if (!isValid) {
      return;
    }
    cashInFromKey.currentState!.save();
                ...
                SharedPrefsHelper.getMsisdn().then((msisdn) => {
                  SharedPrefsHelper.getBasicToken().then((token) => {
                    getOs().then((os)  async {
                      ...
                      var amlBody=AmlBody(amount: amount,currency: "XCD",customerMsisdn: "",destinationMsisdn: phone,keyword:"PMNT",msisdn: msisdn,pin:pinController.text  );
                      ...
                      if(response.responseCode==100){
                        ...
                        Get.to( TransactionOverviewScreen(),arguments: [response,pin,response.keyword, reference.replaceAll(" ", "_"),"","",""]);
                      }else{
                        pinController.text="";
                        DialogHelper.hideLoading();
                        Fluttertoast.showToast(
