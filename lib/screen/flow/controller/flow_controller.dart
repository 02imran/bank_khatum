...
class FlowController extends GetxController with StateMixin {
  final GlobalKey<FormState> flowFromKey = GlobalKey<FormState>();
  ...
  checkTopUp(BuildContext context) {
    final isValid = flowFromKey.currentState!.validate();
    if (!isValid) {
      return;
    }
    flowFromKey.currentState!.save();
    Get.to(const MerchantPaymentScreen(),arguments: [flowMerchantName,flowMerchantCode,amount,"1,1"+phone]);
